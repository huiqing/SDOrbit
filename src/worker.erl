...
vertex_server(StaticMachConf, Credit, Tab, StatData) ->
  IdleTimeout = master:get_idle_timeout(StaticMachConf),
  receive
    {vertex, X, Slot, K} ->
      Credit_plus_K = credit:credit_atomic(K, Credit),
      Now = master:now(),
      {NewCredit, NewTable} = handle_vertex(StaticMachConf, X, Slot, Credit_plus_K, Tab),
    ...
    {dump} ->
      ...
      dump_table(StaticMachConf, Tab, NewStatData)
  after
    IdleTimeout ->
      ...
      vertex_server(StaticMachConf, credit:zero(), Tab, NewStatData)
  end.
...
return_credit(StaticMachConf, Credit, CreditRetd) ->
  case credit:is_zero(Credit) of
    ...
    false -> MasterPid = master:get_master(StaticMachConf),
             case ?SDOrbit of 
                 false ->
                     MasterPid ! {done, Credit}; %% original
                 true ->
                     grouping:s_group_send(MasterPid, {done, Credit}) %% by H.Li
             end,
             CreditRetd + 1
  end.
...
dump_table(StaticMachConf, Table, StatData) ->
    ...
    case ?SDOrbit of 
        false ->
            MasterPid ! {result, PartialOrbit, Stat}; %% original
        true ->
            grouping:s_group_send(MasterPid, {result, PartialOrbit, Stat})
    end.
...
send_vertex(StaticMachConf, X, K) ->
    ...
    case ?SDOrbit of 
        false->
            Pid ! {vertex, X, Slot, K};
        true ->%% original
            grouping:s_group_send(Pid, {vertex, X, Slot, K})
    end,
    ok.

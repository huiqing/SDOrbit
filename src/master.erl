...
do_start_dist([], Acc={NewWorkers, _}) ->
    io:format("Number of worker procs:~p\n", [length(NewWorkers)]),
    Acc;
do_start_dist([{_, 0, _, _, _} | Hosts], Acc) ->
  do_start_dist(Hosts, Acc);
do_start_dist([{Node, M, TabSize, TmOut, SpawnImgComp} |
               Hosts], {Workers, GTabSize}) ->
    case ?SDOrbit of  %%modified by H.Li
        false ->
            Pid = spawn_link(Node, worker, init, [TabSize, TmOut, SpawnImgComp]);
        true ->
            Pid = grouping:s_group_spawn_link(Node, worker, init, [TabSize, TmOut, SpawnImgComp])
    end,
    NewWorkers = [{Pid, GTabSize, TabSize} | Workers],
    NewGTabSize = GTabSize + TabSize,
    Acc = {NewWorkers, NewGTabSize},
...
mk_static_mach_conf(Gs, Master, Workers, GlobalTableSize) ->
   {Gs, Master, Workers, GlobalTableSize, 0, false}. %% true,

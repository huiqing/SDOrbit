...
make_group([Submaster|Workers], Counter) ->
    spawn(Submaster, grouping1, create_group, [self(), [Submaster|Workers], Counter]),
    receive GroupName ->
            {Submaster, GroupName}
    end.
...
create_group_list(Sub_masters, N, NumberOfGroups) ->
    ...
    {ok,master_group,_Submaster_nodes}=
        s_group:new_s_group(master_group, Submaster_nodes),
    %% creates a group and add master and all submasters to it
    List_of_groups=[{Host, GroupName, N div NumberOfGroups} || {Host, GroupName} <- Sub_masters], 
    %% calculate the size that each group is responsible for
    R=N-((N div NumberOfGroups)*NumberOfGroups), %% when N is not divisible by NumberOfGroups -> R>0
    NewList_of_groups=sub_master:change_list(List_of_groups,length(List_of_groups),length(List_of_groups),
                                             3,(N div NumberOfGroups)+R),
    NewList_of_groups.
